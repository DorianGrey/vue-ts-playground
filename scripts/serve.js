...
fs.pathExists(paths.appBuild)
  .then(exists => {
    if (!exists) {
      console.error(`The expected build directory ${chalk.cyan(paths.appBuild)} does not exist. Are you sure you've executed ${chalk.cyan("yarn build")} before?`);
      process.exit(1);
    } else {
